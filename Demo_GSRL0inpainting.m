...
%             sigma = sqrt((Py/10^(BSNR/10)));
    ...
    %%   输入GSR得到的恢复结果 ;
          load GSRbarbaraRatio20.mat ; %% 输入GSR得到的恢复结果 ;
...
%           load GSRparrotsRatio20.mat ;
  ...
  %% 调用 Wavelet Frame deblurring ALgorithm   
     
   if max(x_final(:))>1 %% 使用CSR结果作为初值; 并且将像素值范围调整到（0-255） ；
           x_final = x_final ;
      else
